'''
Prepare more datasets in FewRel JSON format.
...
# FewRel input is a JSON object, where keys are relation IDs and values are lists of entity pairs in context where that relation holds:
# {'tokens': ['words', 'in', 'the', 'sentence'],
#  'h': ['lowercased_head_word', 'entity_id?',
#        [[indices, of head, words]]],
#  't': ['lowercased_tail_word', 'entity_id?',
#        [[indices, of, tail, words]]]}
#
# There's also an accompanying pid2name.json file which give names and
# descriptions for relations in the data.  Not sure what the second field for
# head and tail entities is.


def prep_tacredfmt_data(out_dir: str, in_dir: str, base_in_paths: List[str], rel_key: str='relation', fold_pct: Optional[List[float]]=None):
  ...
  for inp, outp in zip(in_paths, out_paths):
    ...
    with open(inp, 'rt') as f:
      ...
      for o in tacred_objs:
        ...
        head = [htoken, ent_to_id[htoken], [list(range(hs, he+1))], o['subj_type']]
        tail = [ttoken, ent_to_id[ttoken], [list(range(obs, obe+1))], o['obj_type']]
        
        objs[rel_id].append({'tokens': tokens, 'h': head, 't': tail,
                             'id': o['id'],
  ...
  rel_cnts.to_csv(os.path.join(out_dir, 'relation_ent_types.tsv'), header=True, index=False, sep='\t')
...
def prep_tacred(out_dir: str):
  prep_tacredfmt_data(out_dir, TACRED_DIR, ['train.json', 'dev.json', 'test.json'], 'relation')


def prep_cre(out_dir: str):
  prep_tacredfmt_data(out_dir, CRE_DIR, ['challenge_set.train.json', 'challenge_set.dev.json', 'challenge_set.test.json'], 'gold_relation')


def prep_maven(out_dir: str):
  raise NotImplementedException('MAVEN does not appear to include annotations for head & tail entities')


def main():
  for nm, prep_fn in [('tacred', prep_tacred),
                      ('cre', prep_cre),
                      #('maven', prep_maven)
  ]:
